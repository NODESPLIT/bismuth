		...
		Reference get(Reference index) {
			if (index->is(Type::String)) {
				...
				if (is(Type::String)) {
					...
					if (key == "size") {
						return Value::Make(int(as<Array&>().size()));
					}
				} else if (is(Type::Table)) {
					if (key == "size") {
						return Value::Make(int(as<Table&>().size()));
					} else if (key == "keys") {
						Table values = as<Table&>(); Array output;
			...
			if (is(Type::Array)) {
				...
				if (index->is(Type::Range)) {
					...
					Array output;
					if (from <= to) {
						for (int i = from; i <= to; i++) output.push_back(i < 0 || i > array.size() ? Empty() : Copy(array[i]));
					} else {
						for (int i = from; i >= to; i--) output.push_back(i < 0 || i > array.size() ? Empty() : Copy(array[i]));
					}
